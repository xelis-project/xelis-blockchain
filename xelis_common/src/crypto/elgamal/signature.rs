...
impl Signature {
    ...
    pub fn verify(&self, hash: &Hash, key: &PublicKey) -> bool {
        ...
        let calculated = hash_and_point_to_scalar(hash, &r);
        self.e == calculated
    }
}


pub fn hash_and_point_to_scalar(message_hash: &Hash, point: &RistrettoPoint) -> Scalar {
    let mut data = [0u8; 64];
    data[0..32].copy_from_slice(message_hash.as_bytes());
    data[32..64].copy_from_slice(point.compress().as_bytes());
