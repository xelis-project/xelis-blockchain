...
// `delegate` bypasses permission checks, but it must not bypass public/private chunk visibility.
#[tokio::test]
async fn private_chunk_cannot_be_called_via_delegate() {
    let mut state = MockChainState::new();
    let b = create_contract(&mut state, B_PUBLIC_AND_PRIVATE, ContractVersion::V1).expect("create contract B");
    let a = create_contract(&mut state, &code_delegating(&b.to_string(), 1), ContractVersion::V1).expect("create contract A");
    ...
    assert!(!res.is_success(), "delegating to a private chunk should fail even with All permission: {:?}", res);
}
