...
use crate::{block::Block, contract::{ChainState, ModuleMetadata}};
...
pub fn block_current(_: FnInstance, _: FnParams, _: &mut Context) -> FnReturnType<ModuleMetadata> {
    Ok(SysCallResult::Return(Primitive::Opaque(OpaqueWrapper::new(OpaqueBlock)).into()))
}

pub fn block_nonce(_: FnInstance, _: FnParams, context: &mut Context) -> FnReturnType<ModuleMetadata> {
    let block: &Block = context.get().context("current block not found")?;
    Ok(SysCallResult::Return(Primitive::U64(block.get_nonce()).into()))
}

pub fn block_timestamp(_: FnInstance, _: FnParams, context: &mut Context) -> FnReturnType<ModuleMetadata> {
    let block: &Block = context.get().context("current block not found")?;
    Ok(SysCallResult::Return(Primitive::U64(block.get_timestamp()).into()))
}

pub fn block_miner(_: FnInstance, _: FnParams, context: &mut Context) -> FnReturnType<ModuleMetadata> {
    let block: &Block = context.get().context("current block not found")?;
...
pub fn block_hash(_: FnInstance, _: FnParams, context: &mut Context) -> FnReturnType<ModuleMetadata> {
    let state: &ChainState = context.get().context("chain state not found")?;

    Ok(SysCallResult::Return(Primitive::Opaque(OpaqueWrapper::new(state.block_hash.clone())).into()))
}

pub fn block_version(_: FnInstance, _: FnParams, context: &mut Context) -> FnReturnType<ModuleMetadata> {
    let block: &Block = context.get().context("current block not found")?;

    Ok(SysCallResult::Return(Primitive::U8(block.get_version() as u8).into()))
}

pub fn block_tips(_: FnInstance, _: FnParams, context: &mut Context) -> FnReturnType<ModuleMetadata> {
    let block: &Block = context.get().context("current block not found")?;
...
pub fn block_transactions_count(_: FnInstance, _: FnParams, context: &mut Context) -> FnReturnType<ModuleMetadata> {
    let block: &Block = context.get().context("current block not found")?;
    Ok(SysCallResult::Return(Primitive::U32(block.get_txs_count() as _).into()))
}

pub fn block_height(_: FnInstance, _: FnParams, context: &mut Context) -> FnReturnType<ModuleMetadata> {
    let block: &Block = context.get().context("current block not found")?;
    Ok(SysCallResult::Return(Primitive::U64(block.get_height()).into()))
}

pub fn block_extra_nonce(_: FnInstance, _: FnParams, context: &mut Context) -> FnReturnType<ModuleMetadata> {
    let block: &Block = context.get().context("current block not found")?;
