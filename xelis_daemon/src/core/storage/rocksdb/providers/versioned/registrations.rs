...
impl VersionedRegistrationsProvider for RocksStorage {
    ...
    async fn delete_versioned_registrations_at_topoheight(&mut self, topoheight: TopoHeight) -> Result<(), BlockchainError> {
        self.run_blocking_mut(|s| {
            ...
            for res in Self::iter_raw_internal(&s.db, snapshot.as_ref(), IteratorMode::WithPrefix(&prefix, Direction::Forward), Column::PrefixedRegistrations)? {
                let (key, _) = res?;
                Self::remove_from_disk_internal(&s.db, s.snapshot.as_mut(), Column::PrefixedRegistrations, &key)?;
    ...
    async fn delete_versioned_registrations_above_topoheight(&mut self, topoheight: TopoHeight) -> Result<(), BlockchainError> {
        self.run_blocking_mut(|s| {
            ...
            let snapshot = s.snapshot.clone();
            for res in Self::iter_raw_internal(&s.db, snapshot.as_ref(), IteratorMode::From(&prefix, Direction::Forward), Column::PrefixedRegistrations)? {
                let (key, _) = res?;
                Self::remove_from_disk_internal(&s.db, s.snapshot.as_mut(), Column::PrefixedRegistrations, &key)?;
