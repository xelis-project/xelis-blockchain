...
impl VersionedAssetProvider for RocksStorage {
    ...
    async fn delete_versioned_assets_at_topoheight(&mut self, topoheight: TopoHeight) -> Result<(), BlockchainError> {
        ...
        self.run_blocking_mut(|s| {
            ...
            for res in Self::iter_raw_internal(&s.db, snapshot.as_ref(), IteratorMode::WithPrefix(&prefix, Direction::Forward), Column::VersionedAssets)? {
                let (versioned_key, value) = res?;

                Self::remove_from_disk_internal(&s.db, s.snapshot.as_mut(), Column::VersionedAssets, &versioned_key)?;
    ...
    async fn delete_versioned_assets_above_topoheight(&mut self, topoheight: TopoHeight) -> Result<(), BlockchainError> {
        ...
        self.run_blocking_mut(|s| {
            ...
            let snapshot = s.snapshot.clone();
            for res in Self::iter_raw_internal(&s.db, snapshot.as_ref(), IteratorMode::From(&start, Direction::Forward), Column::VersionedAssets)? {
                let (key, value) = res?;
                Self::remove_from_disk_internal(&s.db, s.snapshot.as_mut(), Column::VersionedAssets, &key)?;
    ...
    async fn delete_versioned_assets_below_topoheight(&mut self, topoheight: TopoHeight, keep_last: bool) -> Result<(), BlockchainError> {
        ...
        self.delete_versioned_below_topoheight::<AssetId, Asset>(Column::Assets, Column::VersionedAssets, topoheight, keep_last, |_, v| Ok((v.id, v.data_pointer))).await
    }
}