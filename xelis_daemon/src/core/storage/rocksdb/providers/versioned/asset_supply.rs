...
impl VersionedAssetsCirculatingSupplyProvider for RocksStorage {
    async fn delete_versioned_assets_supply_at_topoheight(&mut self, topoheight: TopoHeight) -> Result<(), BlockchainError> {
        ...
        self.run_blocking_mut(|s| {
            ...
            for res in Self::iter_raw_internal(&s.db, snapshot.as_ref(), IteratorMode::WithPrefix(&prefix, Direction::Forward), Column::VersionedAssetsSupply)? {
                let (versioned_key, value) = res?;

                Self::remove_from_disk_internal(&s.db, s.snapshot.as_mut(), Column::VersionedAssetsSupply, &versioned_key)?;

                let key_without_prefix = &versioned_key[8..];
                let asset_id = AssetId::from_bytes(&key_without_prefix[0..8])?;
                let asset_hash = s.get_asset_hash_from_id(asset_id)?;
    ...
    async fn delete_versioned_assets_supply_above_topoheight(&mut self, topoheight: TopoHeight) -> Result<(), BlockchainError> {
        ...
        self.run_blocking_mut(|s| {
            ...
            let snapshot = s.snapshot.clone();
            for res in Self::iter_raw_internal(&s.db, snapshot.as_ref(), IteratorMode::From(&start, Direction::Forward), Column::VersionedAssetsSupply)? {
                let (key, value) = res?;
                Self::remove_from_disk_internal(&s.db, s.snapshot.as_mut(), Column::VersionedAssetsSupply, &key)?;

                let asset_id = AssetId::from_bytes(&key[8..16])?;
                let hash = s.get_asset_hash_from_id(asset_id)?;
                ...
                if asset.supply_pointer.is_none_or(|v| v > topoheight) {
                    ...
                    if filtered != asset.supply_pointer {
                        ...
                        Self::insert_into_disk_internal(&s.db, s.snapshot.as_mut(), Column::Assets, &hash, &asset)?;
                    }
                }
            }
    ...
    async fn delete_versioned_assets_supply_below_topoheight(&mut self, topoheight: TopoHeight, keep_last: bool) -> Result<(), BlockchainError> {
        ...
        self.delete_versioned_below_topoheight::<AssetId, Asset>(Column::Assets, Column::VersionedAssetsSupply, topoheight, keep_last, |_, v| Ok((v.id, v.supply_pointer))).await
    }
}