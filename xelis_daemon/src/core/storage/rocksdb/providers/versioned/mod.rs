...
impl RocksStorage {
    pub async fn delete_versioned_at_topoheight(&mut self, column_pointer: Column, column_versioned: Column, topoheight: TopoHeight) -> Result<(), BlockchainError> {
        self.run_blocking_mut(|s| {
            let prefix = topoheight.to_be_bytes();
            let snapshot = s.snapshot.clone();
            for res in Self::iter_raw_internal(&s.db, snapshot.as_ref(), IteratorMode::WithPrefix(&prefix, Direction::Forward), column_versioned)? {
                let (key, value) = res?;
                ...
                if pointer.is_some_and(|pointer| pointer >= topoheight) {
                    ...
                    if let Some(prev_topo) = prev_topo {
                        ...
                        Self::remove_from_disk_internal(&s.db, s.snapshot.as_mut(), column_pointer, key_without_topo)?;
                    }
                }
            }
    ...
    pub async fn delete_versioned_above_topoheight(&mut self, column_pointer: Column, column_versioned: Column, topoheight: TopoHeight) -> Result<(), BlockchainError> {
        self.run_blocking_mut(|s| {
            let start = (topoheight + 1).to_be_bytes();
            let snapshot = s.snapshot.clone();
            for res in Self::iter_raw_internal(&s.db, snapshot.as_ref(), IteratorMode::From(&start, Direction::Forward), column_versioned)? {
                let (key, value) = res?;
                ...
                if pointer.is_none_or(|v| v > topoheight) {
                    ...
                    if filtered != pointer {
                        if let Some(pointer) = filtered {
                            Self::insert_into_disk_internal(&s.db, s.snapshot.as_mut(), column_pointer, &key[8..], &pointer.to_be_bytes())?;
                        } else {
                            Self::remove_from_disk_internal(&s.db, s.snapshot.as_mut(), column_pointer, &key[8..])?;
                        }
                    }
                }
            }
    ...
    pub async fn delete_versioned_below_topoheight_default(
        &mut self,
            ...
            if keep_last {
                ...
                for res in Self::iter_raw_internal(&s.db, snapshot.as_ref(), IteratorMode::Start, column_pointer)? {
                    ...
                    while let Some(prev_topo) = prev_version.take() {
                        ...
                        prev_version = s.load_from_disk(column_versioned, &versioned_key)?;
                        if patched {
                            trace!("deleting versioned data at topoheight {}", prev_topo);
                            Self::remove_from_disk_internal(&s.db, s.snapshot.as_mut(), column_versioned, &versioned_key)?;
                        } else if prev_topo < topoheight {
                            trace!("Patching versioned data at topoheight {}", prev_topo);
                            patched = true;

                            let mut data: Versioned<RawBytes> = s.load_from_disk(column_versioned, &versioned_key)?;
                            data.set_previous_topoheight(None);

                            Self::insert_into_disk_internal(&s.db, s.snapshot.as_mut(), column_versioned, &versioned_key, &data)?;
                        }
                    }
                }
                ...
                s.delete_versioned_data_below_topoheight(column_versioned, topoheight)
            }
        })
    }

    fn delete_versioned_data_below_topoheight(&mut self, column: Column, topoheight: TopoHeight) -> Result<(), BlockchainError> {
        // NOTE: Already called within run_blocking_mut from delete_versioned_below_topoheight
