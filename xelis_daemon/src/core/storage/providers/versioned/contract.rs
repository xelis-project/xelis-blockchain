...
pub trait VersionedContractProvider {
    ...
    async fn delete_versioned_contracts_below_topoheight(&mut self, topoheight: TopoHeight) -> Result<(), BlockchainError>;
}
...
impl VersionedContractProvider for SledStorage {
    ...
    async fn delete_versioned_contracts_below_topoheight(&mut self, topoheight: u64) -> Result<(), BlockchainError> {
        trace!("delete versioned contracts below topoheight {}!", topoheight);
        Self::delete_versioned_tree_below_topoheight(&mut self.snapshot, &self.versioned_contracts, topoheight)
    }
}