...
pub struct Block { //add coinbase tx to header
    pub height: u64,
    pub timestamp: u64,
...
impl Block {
    pub fn new(height: u64, timestamp: u64, previous_hash: Hash, difficulty: u64, extra_nonce: [u8; 32], transactions: Vec<Transaction>) -> Self {
        Block {
            height,
            timestamp,
            previous_hash,
...
impl Display for Block {
    fn fmt(&self, f: &mut Formatter<'_>) -> Result {
        write!(f, "Block[height: {}, timestamp: {}, previous_hash: {}, hash: {}, nonce: {}, extra_nonce: {}, txs: {}]", self.height, self.timestamp, hex::encode(self.previous_hash), hex::encode(self.hash), self.nonce, hex::encode(self.extra_nonce), self.transactions.len())
    }
}